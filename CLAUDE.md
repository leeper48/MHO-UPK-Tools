...
UpkMeshScan/     StaticMesh scan, export (FBX + textures), import (FBX -> package), property + material-parameter edits, zone placeholders and whole-zone builds, cross-package copies, level actors, texture import/export incl. .tfc, undo/redo, diagnostics. CLI + WinForms GUI (no args = GUI, dark mode default), AssimpNet. v2.28.1
```
...
- **ICP fog (night harbour mist, confirmed in-game 2026-09-26):** recipe step `--set-property` on `exponentialheightfog_0.exponentialheightfogcomponent_0`: FogMaxOpacity 0.8, StartDistance 1200, FogDensity 0.2, FogHeight 400, OppositeLightColor 110,125,145, LightInscatteringColor 150,145,130 (stock 0.5 / 100 / 0.1 / 764 / 138,182,244 / 222,218,146). MHO's camera sits high, so a start near 100 greys the player's area.
- **ICP sky (confirmed in-game 2026-09-26):** the stock `sm_skysphere` is centred on the map, mirrored top/bottom in UV0: v 1 at the horizon (±6°), v 0 at both poles (`--uv-info` prints v-band elevations). The Brood sky material reads only UV0 and samples the photo at **2 × v** (the city showed twice: horizon and ~45°), so the recipe scales the dome's UV0 v by 0.499 (`--scale-uv ... 1,0.499`; u,v factors). City size is set in the texture, not by moving the dome (`--sky-drop` only sank the city into the height fog): `ICP_Sky.dds` (4096×2048) = `ICP_Sky_photo.dds` twice around below v 0.55 and once above 0.45 (crossfaded), city rows squeezed into the bottom 2.5% (~3°), rows above v 0.25 flattened to their average colour (no pinch at the zenith). A colour-band texture (v bands in distinct colours) settles mapping questions in one screenshot.
- ICP's middle block is random: two rows of 3 cells (x 3456/5760/8064, y 0 and 2304) swap between runs (WareSuper A trio vs Dockyard NESW, FoodTrucks, Dockyard ESW). `IndustryCity_layout.txt` uses the common arrangement (WareSuper at y 0, 3 of 4 logged runs). The building LOD leaves this block out (`--lod-exclude-box 2304,-1152,9216,3456`), since a fixed LOD would be wrong for the other seeds; the ground plane still covers it. `publish/scans/tools/layout.py <logs>` lists what differs.
